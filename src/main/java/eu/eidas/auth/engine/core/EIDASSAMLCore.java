 ...
 * @author fjquevedo
 */
public final class EIDASSAMLCore {

    /** The Constant LOGGER. */
    private static final Logger LOGGER = LoggerFactory
    .getLogger(EIDASSAMLCore.class.getName());
    ...
    private static final String[] AVAILABLE_FORMATS={SAMLExtensionFormat.EIDAS_FORMAT_NAME, SAMLExtensionFormat.STORK1_FORMAT_NAME};
    ...
    */
     ...
     */
    public EIDASSAMLCore(final Properties instance) {
    ...
    private void loadConfiguration(final Properties instance) {
        ...
        try {
            ...
            if (timeNotOnOrAfter.intValue() < 0) {
                LOGGER.error("{} timeNotOnOrAfter is negative number.", SAML_ENGINE_LITERAL );
            ...
            supportedMessageFormatNames=loadSupportedFormats();
    ...
    private Set<String> loadSupportedFormats(){
        ...
        for(String format:AVAILABLE_FORMATS){
            ...
            if(samlCoreProp.containsKey(formatKeyName) && !Boolean.parseBoolean(samlCoreProp.getProperty(formatKeyName))){
                continue;
            }
	...
	if (StringUtils.isBlank(protocolBinding)) {
	    LOGGER.info("ERROR : " + SAML_ENGINE_LITERAL + SAMLCore.PROT_BINDING_TAG
		    + " it's mandatory.");
    ...
    public Set<String> getSupportedMessageFormatNames() {
        return supportedMessageFormatNames;
    }

    public void setSupportedMessageFormatNames(Set<String> supportedMessageFormatNames) {
